...
clear all
%%
data1 = xlsread('dataset5detik.xlsx');
[N1,m1] = size(data1);
Fs1 = 1000;                    %frekuensi sampling Hz
t1 = (0:N1-1)/Fs1;             %time vector
%% EMD FEATURE EXTRATION
tic
hasil = emd(data1(1:5000,1));               %call imf = emd(x) function
hasil = transpose(hasil);                     %transpose matrix after extration
residu = hasil(:,end);
toc
%%
figure(4)
for index=1:3
    subplot(2,2,index), plot(t1,hasil(:, index)), xlim([0 0.1]) ,hold on,...
        title(['IMF ',num2str(index)]),ylabel('Amplitude'),xlabel('Time [s]');
end
 subplot(2,2,4), plot(t1,residu), xlim([0 5]) ,hold on, title(['RESIDU']),xlabel('Time [s]');
...
while (1) % the stop criterion is tested at the end of the loop
   ...
   while SD > 0.2
      ...
      eps = 0.0000000001; % to avoid zero values
